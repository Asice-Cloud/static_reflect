...
struct Student {
  ...
  void speak(std::string lang, bool newline) const {
    if (lang == "cn") {
      std::cout << "你好！我是" << name << "，今年" << age << "岁，是"
                << (sex == 1 ? "男" : "女") << "生。";
    } else {
      std::cout << "Hello! I am " << name << ", " << age << " years old, a "
                << (sex == 1 ? "boy" : "girl") << ".";
    }
    if (newline) {
      std::cout << '\n';
...
template <class T, class N> struct Baby {
  N name;
  T hungry;
};
REFLECT_TYPE_TEMPLATED(((Baby<T, N>), class T, class N), name,
                       hungry); // 如果不能修改类的定义，且类是模板

template <class T> std::string jsonSerialize(T &object) {
  Json::Value root;
  reflect::foreach_member(
      object, [&](const char *key, auto &value) { root[key] = value; });
...
template <class T> T jsonDeserialize(std::string const &json) {
  Json::Value root;
  Json::Reader reader;
